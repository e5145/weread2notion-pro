...
DEFAULT_GATEWAY_SKILL_VERSION = "1.0.4"
...
class GatewayWeReadApi:
    ...
    def _raise_for_gateway_error(api_name, data):
        if "upgrade_info" in data:
            upgrade_info = data.get("upgrade_info") or {}
            message = upgrade_info.get("message", "gateway skill requires upgrade")
            details = [message]
            latest_version = upgrade_info.get("version") or upgrade_info.get("latest_version")
            upgrade_url = upgrade_info.get("upgrade_url") or upgrade_info.get("url")
            if latest_version:
                details.append("latest_version=%s" % latest_version)
            if upgrade_url:
                details.append("upgrade_url=%s" % upgrade_url)
            raise WeReadGatewayError("%s: %s" % (api_name, "; ".join(details)))
